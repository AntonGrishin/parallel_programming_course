...
class Multiplicator {
 ...
 public:
    Multiplicator(const crsMatrix& _A, const crsMatrix& _B,
        std::vector<int>* _columns, std::vector<double>* _values,
...
void MultiplicationTBB(const crsMatrix &A,
    const crsMatrix &B, crsMatrix *C) {
    int N = A.N;
...
int main(int argc, char **argv) {
    ...
    Transp(&B);
    std::cout << "Starting serial alghorithm..." << std::endl;
    tbb::tick_count s2 = tbb::tick_count::now();
    MultiplicationCompare(A, B, &D);
    double serialTime2 = (tbb::tick_count::now() - s2).seconds();

    std::cout << "Starting parallel alghorithm..." << std::endl;
    // parallel version
    tbb::task_scheduler_init init(num_threads);
    tbb::tick_count pT = tbb::tick_count::now();
    MultiplicationTBB(A, B, &C_omp);
    double parallelTime = (tbb::tick_count::now() - pT).seconds();
